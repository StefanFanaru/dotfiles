...
(function() {
    ...
    function prependChromepickerToLinks(links) {
        links.forEach(link => {
            ...
            if (currentHref && !currentHref.includes('chromepicker')) {
                ...
                newLink.setAttribute('href', 'chromepicker:' + currentHref + '@teamslectra');
                //newLink.setAttribute('target','_blank');

                if(link.innerText){
                    newLink.innerText = link.innerText
                }
                else if (link.children && link.children.length && link.children[0].innerText){
    ...
    document.querySelectorAll('[data-tid="file-attachment-grid"]').forEach(attachAttachmentClickListener);
    ...
    const observer = new MutationObserver((mutationsList) => {
        ...
        for (const mutation of mutationsList) {
            ...
            if(node.tagName === 'DIV' && node.getAttribute('data-tid') == 'channel-pane-runway'){
                document.querySelectorAll('[data-tid="file-attachment-grid"]').forEach(attachAttachmentClickListener);
                return
            }
        }
